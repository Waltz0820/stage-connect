...
type SearchActor = { slug: string; name: string; kana?: string | null };
type SearchPlay = { slug: string; title: string; franchise?: string | null };
...
const MIN_Q = 1; // 1文字でも動かしたいなら1。重いなら2に。
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const actorsP = supabase
          .from('actors')
          .select('slug,name,kana')
          .or(`name.ilike.${like},kana.ilike.${like}`)
          .limit(10);

        const playsP = supabase
          .from('plays')
          .select('slug,title,franchise')
          .ilike('title', like)
          .limit(10);

        const [actorsR, playsR] = await Promise.all([actorsP, playsP]);

        if (!alive || my !== seq.current) return;

        const actors = (!actorsR.error ? (actorsR.data ?? []) : []) as any[];
        const plays = (!playsR.error ? (playsR.data ?? []) : []) as any[];

        setResults({
          actors: actors
            .filter((a) => a?.slug && a?.name)
            .map((a) => ({ slug: a.slug, name: a.name, kana: a.kana ?? null })),
          plays: plays
            .filter((p) => p?.slug && p?.title)
            .map((p) => ({ slug: p.slug, title: p.title, franchise: p.franchise ?? null })),
        });
      } catch (e) {
