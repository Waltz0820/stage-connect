...
// key 揺れ吸収（U-NEXT）
const UNEXT_KEYS = ["unext", "u-next", "u_next", "uNext", "U-NEXT", "UNEXT", "U_NEXT"];

function isValidVodUrl(url: string): boolean {
  const s = (url ?? "").trim();
  if (!s) return false;
  // "#" や "javascript:" を弾きたいので http(s) のみ許可
...
const WatchUNextPage: React.FC = () => {
  ...
  const BADGE_CLASS = "bg-neon-purple/10 border-neon-purple/30 text-neon-purple";
  const HOVER_CLASS = "hover:bg-neon-purple/10";
  ...
  const fetchPage = async (nextPage: number) => {
    ...
    const normalized: PlayItem[] = raw
      .map((r) => {
        const url = pickVodUrl(r.vod, UNEXT_KEYS);
        if (!url) return null;
        return {
          id: r.id,
          slug: r.slug,
    ...
    <div className="container mx-auto px-6 pt-8 pb-16 lg:px-8 max-w-5xl animate-fade-in-up">
      ...
      <div className="bg-theater-surface/30 border border-white/10 rounded-2xl overflow-hidden">
        ...
        {!loading && items.length === 0 && (
          <div className="p-10 text-center text-slate-500">まだこの棚に作品がありません（vod のU-NEXTリンクが未入力の可能性）。</div>
        )}
        ...
        {!loading && items.length > 0 && (
          <div className="p-4 sm:p-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {items.map((it) => {
              ...
              const vodHref = it.vodUrl?.trim() ? it.vodUrl.trim() : UNEXT_FALLBACK_URL;
